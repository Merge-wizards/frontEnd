...
function Details({ data, dataBr }) {
  ...
  const fetchFav = () => {
    axios
      .post(`http://localhost:3002/fav`, card[0])
      .then((result) => console.log(result.data))
      .catch((err) => console.log(err));
  };
  ...
  return (
    <div className="wrapper">
      <div>
        ...
        <div className="details-img" src={detailsImg} alt="no-pic" ></div>
      </div>
      <div className="c-d">
        <Card className="img-div">
          <img src={card[0].thumbnail} className="c-image" alt="no-pic" />
          <button className="add-fav-button" onClick={() => fetchFav()}>
          <RiHeartAddFill
        ...
        <div className="column">
          ...
          <p className="description">{card[0].short_description}</p>
          <Link to={card[0].game_url} className='play'><Button >Go to the game</Button></Link>
        </div>
      </div>
